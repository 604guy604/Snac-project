# SNAC Field Intelligence - Master Project Handoff
## Version: Phase 3 Complete / Phase 4 Planned
## Date: May 5 2026
## Session: Polish pass, UI fixes, Phase 4 vision layer architecture locked

---

## Project Summary

SNAC Field Intelligence is a React Native wildlife behavioral inference engine built for field trackers.
A tracker photographs or manually logs animal tracks and environmental conditions. The engine analyzes
the sign and outputs species identification, behavioral classification, follow recommendation, and
layered confidence scores.

Core philosophy: The engine supports the tracker's judgment - it does not replace it.
Computer vision is a servant not a king. Field knowledge always overrides algorithm.

Snack URL: snack.expo.dev/@604appworks/snac-working-build
GitHub: github.com/604guy604/The-Snac-project (private)
SDK: Expo SDK 54
Developer: Sole developer - Samsung S25 FE + borrowed Acer Chromebook (admin locked, browser only)
...
2. No @react-navigation/native-stack in Expo Go SDK 54
3. Navigation via local state only until EAS build
4. Engine always receives and returns metric internally
5. User never sees barometric pressure numbers - trend labels only
6. Hunting pressure always user-entered - algorithm cannot know local pressure
7. Weather auto-pulled from Open-Meteo with manual override
8. JS files are proof of concept - Python/C++ backend is production target
9. Species profile corrections go in snac_profile_enricher.js not snac_profiles.js
10. All confidence outputs must be honest - insufficient data beats false confidence
11. Vision layer is servant not king - feeds engine, never overrides field judgment
12. Session memory has no ceiling - gets smarter with every track
13. Habitat plausibility scored:false = null pass-through, never influences output
14. useSnac.js singleton pattern must be preserved - do not revert to per-component hooks
15. Delete and recreate files in Snack rather than editing large files in place
16. Deliver fixes as complete files not find/replace instructions (Chromebook paste issues)
17. Never edit snac_profiles.js directly - use snac_profile_enricher.js for patches
18. Always read actual profile field shapes before writing SpeciesScreen display logic

---

## Real Field Test Results

Test: Blacktail deer track in deep soft mud, estimated 5 days old by user
Location: Adam's backyard, deep dirt substrate, April, ongoing

| Metric | Before Phase 3 | After Phase 3 |
|--------|---------------|---------------|
| Travel estimate | 10m | 389m |
| Age estimate | None | 32-84 hours |
| Activity prior | None | Bedding (midday crepuscular) |
| Migration status | None | Resident - 1.4km2 cap |
| Follow boost | None | Reduced (midday bedding) |
| Substrate multiplier | 1.0x | 7.56x (mud x overcast x cold) |

---

## Developer Context

Adam is an experienced field tracker whose hands-on domain knowledge directly informs
architectural and logic decisions. He has caught real errors in the engine:
- Inverted snow substrate logic
- Incorrect blacktail deer home range values
His field judgment overrides any algorithmic assumption.

Development constraints: Samsung S25 FE phone + borrowed admin-locked Chromebook.
No terminal access. GitHub repo live at github.com/604guy604/The-Snac-project (private).
Everything built through Expo Snack browser editor and tested via Expo Go on device.
Despite these constraints the app is production-quality.

When Adam gets a proper machine: VS Code, then EAS build, then Phase 4 vision layer.
